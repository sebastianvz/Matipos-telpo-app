...
public class LivenessDetectViewModel extends ViewModel {
    ...
    public List<FaceRectView.DrawInfo> getDrawInfo(List<FacePreviewInfo> facePreviewInfoList, LivenessType livenessType) {
        ...
        for (int i = 0; i < facePreviewInfoList.size(); i++) {
            ...
            // faceid 变化需要检测活体 ，faceid不变  连续3次确认为活体后  不再检测活体;
            if (faceid!=facePreviewInfoList.get(i).getTrackId()){
                faceid=facePreviewInfoList.get(i).getTrackId();
                framenum=0;
                liveness = livenessType == LivenessType.RGB ? facePreviewInfoList.get(i).getRgbLiveness() : facePreviewInfoList.get(i).getIrLiveness();
              //  Log.e("yw_liveness11",faceid+"  "+liveness+"   "+livenessType);
            }else if (framenum<3){
               ...
               // Log.e("yw_liveness22",faceid+"  "+liveness+"   "+livenessType);


            }else {
                //连续3帧活体，不再检测
              //  Log.e("yw_liveness33",faceid+"  "+liveness+"   "+livenessType);
            }



         //   Log.e("yw_liveness",facePreviewInfoList.get(i).getTrackId()+"  "+liveness+"   "+livenessType);
